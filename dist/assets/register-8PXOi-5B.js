import{k as b}from"./main-C6uqHSOu.js";function p(){console.log("✅ register.ts 로드됨");const l=document.getElementById("register-form");if(!l){console.error("❌ 회원가입 폼을 찾을 수 없음");return}l.addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("adminId").value.trim(),o=document.getElementById("password").value.trim(),n=document.getElementById("number").value.trim(),r=document.getElementById("confirmPassword").value.trim(),a=document.getElementById("privacyAgree").checked,i=document.getElementById("kakaoAgree").checked,u=`
